<x-admin-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $area->name }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Area details and usage</p>
            </div>
            <div class="flex items-center gap-3">
                @if(!Auth::user()->isAdminArea())
                <a href="{{ route('areas.edit', $area) }}" class="app-btn-primary px-4 py-2.5 text-sm">
