    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-3">
                @if($selectedRouter)
                    <button onclick="syncProfiles()" class="app-btn-success px-4 py-2.5 text-sm">
                ...
                @endif
                <a href="{{ route('ppp-profiles.create', ['router_id' => $selectedRouter?->id]) }}" class="app-btn-primary px-4 py-2.5 text-sm">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th class="text-left">DNS Server</th>
                                <th class="text-left">Only One</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($profiles as $profile)
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $profile->rate_limit_display }}</td>
                                    <td class="whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $profile->local_address ?? '-' }}</td>
                                    <td class="whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $profile->remote_address ?? '-' }}</td>
                                    <td class="whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $profile->dns_server ?? '-' }}</td>
                                    <td class="whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $profile->only_one_display }}</td>
