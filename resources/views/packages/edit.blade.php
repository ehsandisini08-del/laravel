<x-admin-layout>
    ...
    <div class="max-w-3xl">
        ...
        <form method="POST" action="{{ route('packages.update', $package) }}" class="space-y-6" x-data="packageEditForm()">
            @csrf @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $package->name) }}" required class="mt-1 block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div>
                            ...
                            <input type="number" name="price" id="price" value="{{ old('price', $package->price) }}" required min="0" step="0.01" class="mt-1 block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                    </div>
                    <div>
                        <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
                        <textarea name="description" id="description" rows="2" class="mt-1 block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $package->description) }}</textarea>
                    </div>
                </div>
            </x-card>
                    ...
                    <div>
                        ...
                        <select name="ppp_profile_id" id="ppp_profile_id" required class="mt-1 block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            @foreach($profiles as $profile)
                                <option value="{{ $profile->id }}" {{ old('ppp_profile_id', $package->ppp_profile_id) == $profile->id ? 'selected' : '' }}>{{ $profile->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                        @foreach($areas as $area)
                            ...
                            <label class="flex items-center gap-3 p-3 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors">
                                <input type="checkbox" name="areas[]" value="{{ $area->id }}" {{ $checked ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500">
            ...
            <x-card>
                <label class="flex items-center gap-3 p-3 border border-gray-200 dark:border-gray-600 rounded-lg">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $package->is_active) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500">
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <button type="submit" class="app-btn-primary px-4 py-2.5 text-sm">Update Package</button>
            </div>
        </form>
    </div>
            ...
            return {
                onRouterChange() {
                    ...
                    fetch('/packages/router/' + routerId + '/profiles')
                        .then(r => r.json())
                        .then(profiles => {
                            const currentProfile = '{{ $package->ppp_profile_id }}';
                            profileSelect.innerHTML = '<option value="">-- Select Profile --</option>';
                            profiles.forEach(p => {
                                const selected = (p.id == currentProfile) ? 'selected' : '';
