<x-admin-layout>
    ...
    <div class="max-w-3xl">
        ...
        <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-6" enctype="multipart/form-data" x-data="customerForm()" @submit="saving = true; setTimeout(() => $el.querySelector('button[type=submit]').disabled = true, 0)">
            @csrf
            @method('PUT')
            ...
            <x-card title="Location Coordinates">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="latitude" id="latitude" value="{{ old('latitude', $customer->latitude) }}" required step="any" x-model="lat" @input="updateMarkerFromInputs()" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono">
                    </div>
                    <div>
                        ...
                        <input type="text" name="longitude" id="longitude" value="{{ old('longitude', $customer->longitude) }}" required step="any" x-model="lng" @input="updateMarkerFromInputs()" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono">
                    </div>
                </div>
            ...
            <x-card title="FTTH & Jaringan ODP">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400" x-show="!odpLoading">Hanya menampilkan nomor port yang masih kosong (atau port pelanggan ini).</p>
    ...
    <script>
        function customerForm() {
            return {
                ...
                odpId: '{{ old('odp_id', $customer->odp_id) }}',
                portOdp: '{{ old('port_odp', $customer->port_odp) }}',
                ...
                init() {
                    ...
                    if (this.routerId) {
                        setTimeout(() => this.onRouterChange(), 100);
                    }
                    if (this.odpId) {
                        setTimeout(() => this.onOdpChange(true), 100);
                    }
                },
                ...
                onOdpChange(isInit = false) {
                    ...
                    if (!this.odpId) {
                        this.portOdp = '';
                        portSelect.innerHTML = '<option value="">-- Tidak Terhubung ke ODP --</option>';
                        return;
                    }
                    ...
                    fetch(`/customers/odp/${this.odpId}/available-ports?customer_id=${this.customerId}`)
                        .then(r => r.json())
                        .then(data => {
