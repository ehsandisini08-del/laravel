<x-admin-layout>
    ...
    <div class="max-w-2xl">
        ...
        <form method="POST" action="{{ route('whatsapp.templates.update', $template) }}" class="space-y-6">
            @csrf @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="category" id="category" required class="mt-1 block w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <option value="broadcast" {{ old('category', $template->category) === 'broadcast' ? 'selected' : '' }}>Broadcast</option>
                            <option value="otp" {{ old('category', $template->category) === 'otp' ? 'selected' : '' }}>OTP</option>
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="6" required class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono text-sm">{{ old('content', $template->content) }}</textarea>
                    ...
                    <div>
                        <label class="flex items-center gap-2">
                            ...
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $template->is_active) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500">
