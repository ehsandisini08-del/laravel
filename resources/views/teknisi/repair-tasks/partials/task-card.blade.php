...
<div class="app-card">
    <div class="p-5 space-y-4">
        ...
        <div class="flex items-center gap-2 pt-2">
            ...
            @if($showTakeButton)
                <form method="POST" action="{{ route('teknisi.repair-tasks.take', $task) }}" class="flex-1">
                    @csrf
                    <button type="submit" class="w-full app-btn-primary px-3 py-2 text-sm">
                        Ambil Tugas
                    </button>
                </form>
            @endif
        ...
        <div class="pt-2 border-t border-gray-100 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400">
            ...
            @if($task->takenBy)
                <p class="mt-1">Teknisi: {{ $task->takenBy->name }}</p>
            @endif
        </div>
    </div>
</div>
