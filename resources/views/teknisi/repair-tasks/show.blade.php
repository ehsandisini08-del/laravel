    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    @if($task->takenBy)
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $task->takenBy->name }}</p>
                            @if($task->taken_at)
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ $task->taken_at->format('d M Y, H:i') }}</p>
                            @endif
                        </div>
                    @endif
                </div>
            ...
            @if($task->canBeTakenBy(auth()->user()))
                <x-card>
                    <form method="POST" action="{{ route('teknisi.repair-tasks.take', $task) }}">
                        @csrf
                        <button type="submit" class="w-full app-btn-primary px-4 py-2.5 text-sm">
                            Ambil Tugas Ini
                        </button>
                    </form>
                </x-card>
            ...
            @if($task->canBeCompletedBy(auth()->user()))
                <x-card>
                    <button type="button" onclick="openCompleteModal()" class="w-full app-btn-success px-4 py-2.5 text-sm">
                        Selesaikan Tugas
                    </button>
                </x-card>
            @endif
            ...
            @if(auth()->user()->canManageTeknisiTasks())
                <x-card>
                    <form method="POST" action="{{ route('teknisi.repair-tasks.destroy', $task) }}" onsubmit="return confirm('Yakin ingin menghapus tugas ini?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full app-btn-ghost px-4 py-2.5 text-sm text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20">
                            Hapus Tugas
                        </button>
                    </form>
                </x-card>
